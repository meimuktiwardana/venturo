...
class Mahasiswa 
{
   ...
   public  function get_penjualan($year)
   {
      global $mysqli;
      $query="SELECT trx.*, menu.name AS nama_menu, menu.type AS tipe_menu FROM trx
      INNER JOIN menu ON trx.menu_id = menu.id
      WHERE YEAR(trx.date) = $year";
      $data=array();
      ...
      $response=array(
                     'status' => 1,
                     'message' =>'Get List Penjualan Successfully.',
                     'data' => $data
                  );
